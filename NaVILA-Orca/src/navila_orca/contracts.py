...
@dataclass(frozen=True, slots=True)
class NavigationGuardDecision:
    """Fail-closed decision made from the current navigation camera history."""

    blocked: bool
    reason: str = ""
    obstacle_label: str = "unknown"
    ...
    def __post_init__(self) -> None:
        ...
        label = str(self.obstacle_label).strip() or "unknown"
        object.__setattr__(self, "obstacle_label", label)
        if self.confidence is not None:
            confidence = float(self.confidence)
            if not np.isfinite(confidence) or not 0.0 <= confidence <= 1.0:
                raise ValueError("guard confidence must be between zero and one")
...
@runtime_checkable
class NavigationGuard(Protocol):
    """May stop navigation before the next NaVILA action is executed."""
