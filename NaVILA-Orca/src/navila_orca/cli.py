...
def _run(args: argparse.Namespace) -> int:
    ...
    if args.routeproof_routes:
        ...
        detector = (
            ...
            else AlwaysClearDetector()
        )
        routeproof_agent = RouteProofAgent(
            route_plan,
            detector,
    ...
    try:
        ...
        runner = NavigationRunner(
            ...
            navigation_guard=routeproof_agent,
        )
        if routeproof_agent is None:
            result = runner.run(episode)
        else:
            routeproof_mission = routeproof_agent.verify(runner, episode)
            result = routeproof_mission.last_run
        ...
        payload = {
            ...
            "routeproof": (
                None if routeproof_mission is None else routeproof_mission.as_dict()
            ),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    instruction_group.add_argument(
        "--routeproof-routes",
        help="JSON file containing RouteProof's ordered, pre-approved routes",
    )
    run.add_argument(
    ...
    run.add_argument(
        ...
        default=False,
        help="require both NaVILA stop and the goal metric before verifying a route",
    )
